...
const string STORAGE_KEY = "yojek_ops_data_v1";
...
class YojekStore {
 public:
  YojekData read() const { return data; }

  void write(const YojekData& newData) { data = newData; }

  Courier upsertCourier(Courier profile) {
    YojekData current = read();
    Courier courier;

    courier.id = profile.id.empty() ? "KURIR-UTAMA" : profile.id;
    courier.username = trim(profile.username);
    courier.name = trim(profile.name.empty() ? "Kurir Yojek" : profile.name);
  ...
  YojekData toAnalyticsPayload() const { return read(); }
 ...
 private:
  ...
  static vector<Order>::iterator findOrder(YojekData& current,
